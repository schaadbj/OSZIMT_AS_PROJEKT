...
//	private Separator notvisible;
	...
	@FXML
	private Spinner<Integer> delflu_input_id;
	@FXML
	private Pane showflu_pane;
	@FXML
	private Button showflu_button;
	@FXML
	private Spinner<Integer> showflu_input_id;
	@FXML
	private TableView<Flugzeug> showflu_table;
	@FXML
	...
	@FXML
	private TableView<Flugzeug> allflu_table;
	@FXML
...
//	private Separator notvisible;
	...
	public void setNotVisible() {
		...
		if ((label_output.getText() != null) & (label_output.getText() != ""))label_output.setText("");
	}
	public void initSpinner(int initialValue, int min, int max, Spinner<Integer> spinner) {
		//Setzt Werte für Spinner 1-1000, Startwert=1
...
//=====NEUES FLUGZEUG===============================	
	...
	public void menu_newflu_klick(ActionEvent event) {
		setNotVisible();
		//Initialisiert Spinner (Initialwert, Minwert, Maxwert, Spinnerid)
		initSpinner(1, 1, 1000, newflu_input_sitze);		
	...
	public void newflu_button_klick(ActionEvent event) throws SQLException {
		...
		int sitze = newflu_input_sitze.getValue();
	    ...
	    try {
			...
			//erzeugt Flugzeug Objekt & speichert in DB
		   	BuilderDB.newflu(id, modell, sitze);
		   	label_output.setText("Flugzeug gespeichert.");
		}
	}
...
//=====FLUGZEUG LÖSCHEN===============================	
	...
	public void menu_delflu_klick(ActionEvent event) throws SQLException {
		setNotVisible();
		//Initialisiert Spinner (Initialwert, Minwert, Maxwert, Spinnerid)
		initSpinner(1, 1, 1000, delflu_input_id);	
	...
	public void delflu_button_click(ActionEvent event) throws SQLException {
		//Liest Wert ein
		int id = delflu_input_id.getValue();
		...
		for (Flugzeug flugzeug: flugzeuge) {
			if (id==flugzeug.getId()) {
				...
				alert.setTitle("Flugzeug löschen");
				alert.setHeaderText("Wollen sie folgendes Flugzeug wirklich löschen:");
				alert.setContentText("Flugzeugnummer: "+id +"\nModell: "+flugzeug.getModell()+"\nSitzplätze: "+flugzeug.getSitze());
				
				Optional<ButtonType> result = alert.showAndWait();
				if (result.get() == ButtonType.OK) {
					//Löschen
					BuilderDB.delflu(id);
					label_output.setText("Flugzeug gelöscht.");
				}//end of if
				break;
			}//end of if
		}//end of for
	}//end of delflu_button_click	
...
//=====FLUGZEUG ANZEIGEN============================	
	...
	@FXML
	public void menu_showflu_klick(ActionEvent event) throws SQLException {
		setNotVisible();
		//Initialisiert Spinner (Initialwert, Minwert, Maxwert, Spinnerid)
		initSpinner(1, 1, 1000, showflu_input_id);
		
		showflu_pane.setVisible(true);
	}
	// Event Listener on Button[#showflu_button].onAction
	@FXML
	public void showflu_button_klick(ActionEvent event) throws SQLException {
		//Liest Wert ein
		int id = showflu_input_id.getValue();
		//erzeugt Flugzeug Objekte
		List<Flugzeug> flugzeuge=BuilderDB.getflu();
		for (Flugzeug flugzeug: flugzeuge) {
			if (id==flugzeug.getId()) {
				showflu_table.getItems().add(flugzeug);
				System.out.print(flugzeug.getId()+flugzeug.getModell()+flugzeug.getSitze());
				//ObservableList<O_Flugzeug> data = FXCollections.observableArrayList();
				//data.add(flugzeug);
			}
		}//end of for
	}		
...
//Menüauswahl
	...
	@FXML
	public void menu_allflu_klick(ActionEvent event) throws SQLException {
		setNotVisible();
		
		List<Flugzeug> flugzeuge=BuilderDB.getflu();
		//allflu_table.getItems().add(flugzeuge);
